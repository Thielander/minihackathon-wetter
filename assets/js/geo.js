document.addEventListener("DOMContentLoaded", function() {
    ...
    function errorCallback(error) {
        ...
        document.getElementById('placeholder-content').textContent = "Es ist nicht möglich, die aktuelle Position zu ermitteln. Erlaube die Stanortbestimmung um aktuelle Daten zu erhalten. Ohne die Standortbestimmung entgeht dir eine KI Kleidungs- und Aktivitätsempfehlung.";
    }
            ...
            .then(data => {
                ...
                if (placeholderHeading && placeholderContent) {
                    ...
                    placeholderContent.innerHTML = `
                        ...
                        <p id="loading-text">Werden geladen...</p>
                    `;
    ...
    function fetchOpenAIRecommendations(weatherData) {
        ...
        fetch(`openai.php?temperature=${weatherData.temperature}&wind_speed=${weatherData.wind_speed}&precipitation_probability=${weatherData.precipitation_probability}`)
            ...
            .then(data => {
                ...
                if (loadingText) {
                    loadingText.remove(); // Entfernt den "Werden geladen..." Text
                }
                ...
                `;
                placeholderContent.appendChild(recommendations);
            })
            .catch(error => {
                console.error('OpenAI fetch error:', error);
                const loadingText = document.getElementById('loading-text');
                if (loadingText) {
                    loadingText.textContent = 'Fehler beim Abrufen der Empfehlungen von OpenAI.';
                }
            });
    }
